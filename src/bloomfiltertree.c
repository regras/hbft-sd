...
/* deve receber o número da folha diretamente e não do nó!*/
...
void destroy_bf_tree(BLOOMFILTER_TREE *bft) {
    ...
    while (i < bft->size){
        //printf("%d\n",bft->size);
        if(is_leaf(bft,i)){
            if(get_leaf_bf(bft,i-bft->size/2)->file_name != NULL) {
                //printf("name %s\n",get_leaf_bf(bft,i-bft->size/2)->file_name);
                free(get_leaf_bf(bft,i-bft->size/2)->file_name);
                freed_names++;
            }
    ...
    /* if the printed number is different from the number of files you have inserted something went wrong. There must be only one file per leaf*/
#ifdef DEBUG
    printf("total freed leaves w/ allocated names = %d\n",freed_names);
#endif
#ifdef FINGERPRINT_LEAVES
